...
with open('../data/random ODEmu_0.1.pkl', 'rb') as f: 
     error1,normxtot1,normltot1, normutot1 = pickle.load(f)
     
with open('../data/random ODEmu_1.pkl', 'rb') as f: 
     error2,normxtot2,normltot2, normutot2 = pickle.load(f)

with open('../data/random ODEmu_5.pkl', 'rb') as f: 
     error3,normxtot3,normltot3, normutot3 = pickle.load(f)

with open('../data/random ODEmu_10.pkl', 'rb') as f: 
...
ax_soln[0].plot(error3[:, 0]/normxtot3, label='$\mu=5$')
ax_soln[0].plot(error4[:, 0]/normxtot4, label='$\mu=10$')
...
ax_soln[1].plot(error3[:, 1]/normltot3, label='$\mu=5$')
ax_soln[1].plot(error4[:, 1]/normltot4, label='$\mu=10$')
...
ax_soln[2].plot(error3[:, 2]/normutot3, label='$\mu=5$')
ax_soln[2].plot(error4[:, 2]/normltot4, label='$\mu=10$')

ax_soln[2].set_title("rel. err. control")

ax_soln[1].legend(loc="upper center", bbox_to_anchor=(0.5, 1.58), ncol=4)

for i in range(3):
    ax_soln[i].set_xticks([0,500,1000])
...
plt.savefig('figs/random.png', bbox_inches='tight', dpi=500)
